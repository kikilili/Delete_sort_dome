...
#import "ExpertsListItem.h"
#import "Masonry.h"

@interface ExpertsScrollView ()
...
@property (nonatomic,strong) ExpertsListItem *itemSelect;


@end

@implementation ExpertsScrollView
...
#pragma 标签
    ...
    recommendedclasslabel.text = @"专家推荐";
    ...
    for (int i =0; i <count1; i++) {
        ...
        // **********************   控制按钮的frame值  **********************
        ExpertsListItem *item = [[ExpertsListItem alloc] initWithFrame:CGRectMake(padding+(padding+kItemW)*(i% itemPerLine), padding+(kItemH + padding)*(i/itemPerLine) + self.headeroneview.frame.size.height, kItemW, kItemH)];
        item.longPressBlock = ^(){
    ...
    for (int i=0; i<count2; i++) {
        ExpertsListItem *item = [[ExpertsListItem alloc] initWithFrame:CGRectMake(padding+(padding+kItemW)*(i%itemPerLine),CGRectGetMaxY(self.headertwoview.frame)+padding+(kItemH+padding)*(i/itemPerLine), kItemW, kItemH)];
        item.longPressBlock = ^(){
            if (weekSelf.longPressedBlock) {
                weekSelf.longPressedBlock();
            }
...
-(void)itemRespondFromListBarClickWithItemName:(NSString *)itemName{
    for (int i = 0 ; i<self.allItemsarray.count; i++) {
        ExpertsListItem *item = (ExpertsListItem *)self.allItemsarray[i];
